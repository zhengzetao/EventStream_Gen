"""Weibull inter-event time regime."""

from __future__ import annotations

import random
from typing import Any, Dict

from .base import TemporalRegime, positive_number
from event_stream_generator.temporal.history import HistoryState


class WeibullRegime(TemporalRegime):
    name = "weibull"

    def sample(
        self, params: Dict[str, Any], history_state: HistoryState, rng: random.Random
    ) -> float:
        del history_state
        if not self.validate_params(params):
            raise ValueError("weibull regime requires shape > 0 and scale > 0")
        return max(
            rng.weibullvariate(float(params["scale"]), float(params["shape"])),
            1e-9,
        )

    def validate_params(self, params: Dict[str, Any]) -> bool:
        return positive_number(params, "shape") and positive_number(params, "scale")
